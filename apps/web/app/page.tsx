...
import type { ConsumerLocation, FarmLot, GeofenceLot, OrderStatus, QuoteInput, QuoteSnapshot, Role } from "@farmit/domain";
...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        const state = JSON.parse(saved) as { selectedLotId?: string; quoteInput?: QuoteInput; quote?: QuoteSnapshot | null; decision?: Decision; orderStatus?: OrderStatus | null };
        if (state.selectedLotId) setSelectedLotId(state.selectedLotId);
        if (state.quoteInput) setQuoteInput(state.quoteInput);
        setQuote(state.quote ?? null); setDecision(state.decision ?? null); setOrderStatus(state.orderStatus ?? null);
      }
  ...
  useEffect(() => {
    if (hydrated) localStorage.setItem(demoKey, JSON.stringify({ selectedLotId, quoteInput, quote, decision, orderStatus }));
  }, [hydrated, selectedLotId, quoteInput, quote, decision, orderStatus]);
  ...
  useEffect(() => { void refreshCatalog(); /* eslint-disable-line react-hooks/exhaustive-deps */ }, []);

  function switchRole(nextRole: Role) { setRole(nextRole); setView("overview"); setNotice(""); }
  async function resetDemo() { localStorage.removeItem(demoKey); setSelectedLotId(initialLot.id); setDraftLot(draftLotTemplate); setQuoteInput(initialQuote); setQuote(null); setDecision(null); setOrderStatus(null); setNotice("Demo reset. Start by listing a farmer lot."); setRole("farmer"); setView("workspace"); await refreshCatalog(); }
  ...
  async function submitDecision(nextDecision: Exclude<Decision, null>) { if (!liveQuote) return; setLoading(true); try { const response = await fetch("/api/orders", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ quoteId: liveQuote.id, decision: nextDecision }) }); if (!response.ok) throw new Error(); setDecision(nextDecision); setOrderStatus(nextDecision === "accepted" ? "reserved" : null); setNotice(nextDecision === "accepted" ? "Demo reservation confirmed. No payment has been taken." : "Offer declined. The price snapshot remains unchanged."); } catch { setNotice("That decision could not be saved. Try again."); } finally { setLoading(false); } }
  async function advanceDelivery() { if (!orderStatus || orderStatus === "delivered") return; const next: Record<OrderStatus, OrderStatus> = { reserved: "milling", milling: "in_transit", in_transit: "delivered", delivered: "delivered" }; const nextStatus = next[orderStatus]; setLoading(true); try { const response = await fetch("/api/orders", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ quoteId: liveQuote?.id, status: nextStatus }) }); if (!response.ok) throw new Error(); setOrderStatus(nextStatus); setNotice(`Delivery advanced to ${statusLabel(nextStatus)}.`); } catch { setNotice("Delivery status could not be advanced."); } finally { setLoading(false); } }
  ...
  return <main className="app-shell">
    ...
    <div className="workspace"><aside className="sidebar"><div className="profile"><div className="avatar">{role === "farmer" ? "S" : role === "operator" ? "O" : "A"}</div><div><strong>{role === "farmer" ? "Shivanna" : role === "operator" ? "FarmIt team" : "Ananya Rao"}</strong><span>{copy[role]}</span></div></div><nav><button className={`nav-item ${view === "overview" ? "active" : ""}`} onClick={() => setView("overview")}><span>◈</span> Overview</button><button className={`nav-item ${view === "workspace" ? "active" : ""}`} onClick={() => setView("workspace")}><span>□</span> {workspaceLabel}</button><button className={`nav-item ${view === "account" ? "active" : ""}`} onClick={() => setView("account")}><span>◇</span> Account</button></nav><div className="side-note"><span className="tiny-sun">✦</span><strong>Small batch,<br />clear numbers.</strong><p>Saved locally for this demo. Supabase replaces this store when the pilot is connected.</p></div></aside>
      <section className="content"><div className="role-switcher"><span>Signed in as</span>{(["consumer", "farmer", "operator"] as Role[]).map((item) => <button key={item} className={role === item ? "selected" : ""} onClick={() => switchRole(item)}>{item}</button>)}<button className="language" onClick={() => setLocale(locale === "en" ? "kn" : "en")}>{locale === "en" ? "ಕನ್ನಡ" : "English"}</button></div>
        {view === "account" ? <AccountPanel role={role} onReset={resetDemo} /> : role === "farmer" ? <FarmerPanel draft={draftLot} setDraft={setDraftLot} onSave={saveLot} loading={loading} /> : role === "operator" ? <OperatorPanel lots={nearbyLots} selectedLotId={selectedLotId} onSelectLot={setSelectedLotId} input={quoteInput} setInput={setQuoteInput} onGenerate={generateQuote} loading={loading} quote={quote} /> : <ConsumerPanel lots={nearbyLots} outsideCount={outsideCount} selectedLotId={selectedLotId} onSelectLot={setSelectedLotId} selectedLot={selectedLot} quote={liveQuote} hasSnapshot={Boolean(quote)} decision={decision} orderStatus={orderStatus} onDecision={submitDecision} onAdvance={advanceDelivery} onOpenOperator={() => switchRole("operator")} loading={loading} copy={copy} />}
...
function OperatorPanel({ lots, selectedLotId, onSelectLot, input, setInput, onGenerate, loading, quote }: { lots: GeofenceLot[]; selectedLotId: string; onSelectLot: (lotId: string) => void; input: QuoteInput; setInput: (input: QuoteInput) => void; onGenerate: () => void; loading: boolean; quote: QuoteSnapshot | null }) {
