...
- **Decision:** `apps/web/lib/catalog.ts` holds a seeded 14-lot catalog in a module-level array; `GET /api/lots` reads it, `POST /api/lots` appends (validated). Farmer-listed lots are lost on server restart — accepted for the demo. The swap to Supabase `farm_lots` is documented step-by-step in `DEPLOYMENT.md` (Environment 2).
